...
import { ongoingDriverReqCancelAction } from "../../../../redux/actions/manageBookings/onGoingBookingAction";
import SpinnerLoading from "../../../utilits/spinnerLoading";

function DriverRequestedCancelModal({
  driverRequestedCancelShow,
  handleDriverRequestedCancelClose,
  ongoingBookingData,
  ongoingTable,
  setOngoingTable,
}) {
  ...
  const [DriverCancellationFee, setDriverCancellationFee] =
    useState("nocancellationFee");
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ConfirmPassword: ``,
      cancellationReason: ``,
      driverCancellationFee: `nocancellationFee`,
    },
    validationSchema: Yup.object({
      ...
      driverCancellationFee: Yup.string(),
    }),
    onSubmit: (values, { resetForm }) => {
      setLoading(true);
      dispatch(
        ongoingDriverReqCancelAction(
          {
            ...
            cancellation_fee_charged:
              values?.driverCancellationFee === "nocancellationFee"
                ? false
                : true,
          },
          onSucess,
  ...
  function handleDriverCancellationFeeButton(e) {
    setDriverCancellationFee(e.target.value);
    let event = {
      target: {
      ...
      {data?.map((items) => (
        <React.Fragment key={items?.bookingID}>
          ...
          <div className="row gx-0">
            <div className="col-4">
              <div className="text-start ps-2">
                <span className="primary_color fs_14 text-nowrap">Booking Status</span>
              </div>
            </div>
            <div className="col-1">
              <div>
  ...
  return (
    <>
      ...
      <Modal
        centered
        show={driverRequestedCancelShow}
        onHide={handleDriverRequestedCancelClose}
              ...
              <div className="ms-5">
                <div className="d-flex  justify-content-left ms-5 gap-1">
                  <input
                    ...
                    id="cancellationFee"
                    checked={DriverCancellationFee === "cancellationFee"}
                    value="cancellationFee"
                    onChange={handleDriverCancellationFeeButton}
                  />
                  <label
                ...
                <div className="d-flex justify-content-left ms-5 gap-1">
                  <input
                    ...
                    checked={DriverCancellationFee === "nocancellationFee"}
...
export default DriverRequestedCancelModal;
