...
const ComplaintChatSection = ({
  ...
  console.log(driverID, "adasdadadasdasd");
  ...
  const handleSend = async () => {
    if (currentMessage !== "" || file !== null) {
      ...
      await socket.emit("complaint_send_message", messageData);
      chatListFnc();
      setCurrentMessage("");
      setFile(null);
      setPage(0);
  ...
  useEffect(() => {
    socket.on("complaint_message_received", (data) => {
      console.log(data, "complaint_message_received");
      setChangesMade(data);
      setShouldScrollToBottom(true);
    ...
    return () => {
      socket.off("receive_complaint_driver_message");
    };
  }, [socket, roomChange]);
  ...
  useEffect(() => {
    if (type === "driverComplaints" || type === "riderComplaints") {
      chatListFnc();
    }
  }, [roomChange, changesMade, page, socket]);
  ...
  const onSuccess = (data) => {
    ...
    // setStoredMessages(... (data?.data?.data).reverse());
    // setStoredMessages((prevMessages) =>
    //   [...prevMessages.reverse(), ...data?.data?.data].reverse()
    // );
    // setShouldScrollToBottom(true);
  };
  const onError = (data) => {
