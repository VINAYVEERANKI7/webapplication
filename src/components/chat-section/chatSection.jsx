...
const ChatSection = ({
  ...
  console.log(file, "imagedataaaaaa");
  ...
  console.log(adminUserName, "adasdadsa");
  ...
  useEffect(() => {
    if (type === "riderSos") {
      socket.on("rider_sending_sos_msg_to_admin", (data) => {
        console.log(data, "rider_sending_sos_msg_to_admin");
        setChangesMade(data);
        setShouldScrollToBottom(true);
        ...
        // socket.emit("sos_message_read", isMessageSosRead);
      });
  ...
  // useEffect(() => {
  //   socket.on("receive_sos_rider_message", (data) => {
  //     console.log(data, "receive_sos_rider_message");
  ...
  useEffect(() => {
    if (type === "driverSos" || type === "riderSos") {
      setLoading(true);
      chatListFnc();
    }
  }, []);
  ...
  const onSuccess = (data) => {
    ...
    // setStoredMessages((prevMessages) => [...prevMessages, ...data?.data?.data]);
    setShouldScrollToBottom(true);
  ...
  console.log(parseInt(allMessageCount / 15), "kahajsd");
  console.log(storedMessages, "storedMessages");
          ...
          <div className="w-100">
            {!file && fileLoading === true ? (
              ...
              <div className="position-relative width_fit">
                {
                  // (file?.imgType === "image/jpeg" ||
                  //   file?.imgType === "application/pdf")
                  file && (
                    <i
                ...
                {file && (
                  <>
                    ...
                    >
                      <span className="fs_20 primary_color">
                        {(file?.imgName).substring(0, 8) + "......."}
                      </span>{" "}
                      (document)
                    </div>
                  ) : file?.imgType === null ||
                    file?.imgType === undefined ? null : (
